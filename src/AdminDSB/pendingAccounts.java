...
public class pendingAccounts extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        declineBtn.setFont(new java.awt.Font("Yu Gothic", 1, 11)); // NOI18N
        ...
        acceptBtn.setFont(new java.awt.Font("Yu Gothic", 1, 11)); // NOI18N
    ...
    private void displayData() {
        try {
            ResultSet rs = new DBConnector().getData("select id,email,username,type from john where status = 'PENDING'");
    ...
    private void acceptAccount() {
        ...
        if (rowIndex < 0) {
            ...
            try {
                ...
                new DBConnector().updateData("UPDATE john SET status = 'ACTIVE' WHERE id = '" + tbl.getValueAt(rowIndex, 0).toString() + "'");
    ...
    private void declineAccount() {
        ...
        if (rowIndex < 0) {
            ...
            try {
                TableModel tbl = pendings.getModel();
                new DBConnector().updateData("UPDATE john SET status = 'DECLINED' WHERE id = '" + tbl.getValueAt(rowIndex, 0).toString() + "'");
